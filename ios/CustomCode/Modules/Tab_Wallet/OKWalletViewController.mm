...
- (void)viewDidLoad {
    ...
    //创建钱包
//    NSString *passwordCreateHD = @"123456";
//    NSString *seed = @"";
//    NSDictionary *createHD =  [kPyCommandsManager callInterface:kInterfaceCreate_hd_wallet parameter:@{@"password":passwordCreateHD,@"seed":seed}];
//    NSLog(@"createHD == %@",createHD);
    
    [kPyCommandsManager callInterface:kInterfaceLoad_wallet parameter:@{@"name":@"BTC-5"}];
...
//    [self get_all_tx_list];
    ...
    //最新封装
    //获取交易信息
    //[kPyCommandsManager callInterface:kInterfaceGet_tx_info parameter:@{@"tx_hash":@"8be667f21efd7003186b8a98d163dd5a2a06771d5a554393b1b0d72d80b0a6f7"}];
    
    
    //获取默认费率
    NSString *fee_ =  [kPyCommandsManager callInterface:kInterfaceGet_default_fee_status parameter:@{}];
    NSLog(@"fee_ == %@",fee_);
    ...
    //输入地址和转账额度 获取fee
    NSDictionary *outputsDict = @{@"bcrt1q0f9n7zazc7kltjtf4sa4mdv8ds04lwffqn8vcr":@"2"};
    NSArray *outputsArray = @[outputsDict];
    NSString *outputs = [outputsArray mj_JSONString];
    NSString *memo = @"s";
    NSDictionary *dict =  [kPyCommandsManager callInterface:kInterfaceGet_fee_by_feerate parameter:@{@"outputs":outputs,@"message":memo,@"feerate":@"180"}];
    NSLog(@"dict == %@",dict);

    NSDictionary *dict1 =  [kPyCommandsManager callInterface:kInterfaceMktx parameter:@{@"outputs":outputs,@"message":memo}];
    ...
    NSLog(@"signTxDict == %@",signTxDict);

    NSString *signTx = [signTxDict safeStringForKey:@"tx"];
    NSDictionary *dictBroadcast_tx = [kPyCommandsManager callInterface:kInterfaceBroadcast_tx parameter:@{@"tx":signTx}];
    NSLog(@"dictBroadcast_tx == %@",dictBroadcast_tx);
}
...
- (void)create_hd_wallet
{
    ...
    PyObject *result = NULL;
    result = PyObject_CallMethod(self.pyInstance, [@"create_hd_wallet" UTF8String], "(s)", [@"123456" UTF8String]);
    if (result == NULL) {
         PyErr_Print();
    }
    char * resultCString = NULL;
...
//    }
    ...
    PyObject *result = NULL;
    result = PyObject_CallMethod(self.pyInstance, [@"load_wallet" UTF8String], "(s)",[@"BTC-20" UTF8String]);
    if (result == NULL) {
        PyErr_Print();
    }
    char * resultCString = NULL;
    ...
    result1 =  PyObject_CallMethod(self.pyInstance, [@"select_wallet" UTF8String], "(s)",[@"BTC-20" UTF8String]);
    if (result1 == NULL) {
        PyErr_Print();
    }
    char * resultCString1 = NULL;
...
- (IBAction)tableViewHeaderAddBtnClick:(UIButton *)sender {
    ...
    [self presentViewController:biologicalVc animated:YES completion:nil];
